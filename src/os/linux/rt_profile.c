...
NDIS_STATUS	RTMPReadParametersHook(RTMP_ADAPTER *pAd)
{
	...
	if (src && *src)
	{
		...
		if (IS_FILE_OPEN_ERR(srcf)) 
		{
			...
			//open outside dat file failed , load param from default structure
			if (default_profile_buffer != NULL)
			{
				DBGPRINT(RT_DEBUG_ERROR, ("###########################################\n"));
				DBGPRINT(RT_DEBUG_ERROR, ("###########################################\n"));
				DBGPRINT(RT_DEBUG_ERROR, ("Read Parameters from default_profile_buffer\n"));
				DBGPRINT(RT_DEBUG_ERROR, ("###########################################\n"));
				DBGPRINT(RT_DEBUG_ERROR, ("###########################################\n"));
				RTMPSetProfileParameters(pAd, default_profile_buffer);
				retval = NDIS_STATUS_SUCCESS;
			}
			else
			{
				DBGPRINT(RT_DEBUG_ERROR, ("Read default_profile_buffer failed!\n"));
	...
	IN UCHAR OpMode)
{
	...
	/*
	trying to achieve 	|ether | radiotap | ip | payload | format
	this code must be after RtmpOsPktProtocolAssign , where pRxPkt->data will be pointing at the start of ip header
	*/
	if(pAd->flg_radiotap_in_data_on)
	{
		struct sk_buff *pPkt = RTPKT_TO_OSPKT(pRxPkt);
		
		UCHAR *combine_radiotap_and_data_buf=NULL;
		os_alloc_mem(NULL, &combine_radiotap_and_data_buf, (pPkt->len + pAd->data_radiotap_header_len));
		NdisCopyMemory(combine_radiotap_and_data_buf, pAd->data_radiotap_header, pAd->data_radiotap_header_len);
		NdisCopyMemory(combine_radiotap_and_data_buf + pAd->data_radiotap_header_len, pPkt->data, pPkt->len);

		//printk("@@@@@ headroom: %d \n",skb_headroom(pPkt));
		hex_dump("#####  pRxPkt->head : ",pPkt->head,skb_headroom(pPkt));
		hex_dump("#####  pRxPkt->data : ",pPkt->data,pPkt->len);
	
		skb_put(pRxPkt,pAd->data_radiotap_header_len);
		
		NdisCopyMemory(pPkt->data, combine_radiotap_and_data_buf, pPkt->len);
		hex_dump("#####  head after push : ",pPkt->head,skb_headroom(pPkt));
		hex_dump("#####  data after push : ",pPkt->data,pPkt->len);
		os_free_mem(NULL,combine_radiotap_and_data_buf);
	}
